...
{
    [Header("Prefabs for different types of trash bins")]
    ...
    // Queue of bins in the scene
    List<TrashBins> queueOfBins;
    // List of trash items to distribute in bins
    List<TrashItems> listOfTrashItems;
    // Max number of items per bin
    public int maxBinCapacity = 4; // Space between bins
    public float distanceBetweenBins = 1.5f;
    public bool phase = true;  // True for first phase (garbage Sorting), false for second phase (Runner Game)

    // Main function to generate bins for sorting phase
    public void GenerateBins()
    {
        if (GameManager.Instance == null )
        {
            Debug.LogError("GameManager or GarbageItemManager is null!");
            return;
        }
    ...
    // Populate the list of trash items based on phase
    void PopulatelistofTrashItems()
    {
        if (phase)
        {
            foreach (GameObject obj in GameManager.Instance.garbageItemManager.pickUpsList)
            {
    ...
    // Generate bins based on a list of trash items
    void GenerateBins(List<TrashItems> listOfItems)
    {
        ...
        // Get number of bins required for each type
        ...
        // Instantiate bins for each type
        ...
        ShuffleList(newBinsBatch); // Randomize bin order
        queueOfBins.AddRange(newBinsBatch);

    }

    // Remove the first bin in the queue after it is used
    public void RemoveBin(TrashBins bin)
    {
        ...
        // If no bins left, check scene and trigger next scene
    ...
    // Returns a list with the number of bins needed based on item count and max bin capacity
    ...
    // Instantiate bins with specific capacities and type
    ...
    // Position bins in the scene based on queue order
    ...
    // Animate the queue moving forward
